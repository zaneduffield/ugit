...
def parse_args():
    ...
    log_parser.add_argument('oid', nargs='?', type=oid, default=data.HEAD)

    show_parser = commands.add_parser('show')
    show_parser.set_defaults(func=show)
    show_parser.add_argument('oid', nargs='?', type=oid, default=data.HEAD)

    diff_parser = commands.add_parser('diff')
    diff_parser.set_defaults(func=_diff)
    diff_parser.add_argument('--cached', action='store_true')
    diff_parser.add_argument('commit', nargs='?', type=oid, default=data.HEAD)
    ...
    tag_parser.add_argument('oid', nargs='?', type=oid, default=data.HEAD)

    branch_parser = commands.add_parser('branch')
    branch_parser.set_defaults(func=branch)
    branch_parser.add_argument('name', nargs='?')
    branch_parser.add_argument('start_point', default=data.HEAD, type=oid, nargs='?')
...
def init(args):
    base.init()
    print(f'Initialised empty ugit repository in {os.getcwd()}')
...
def _diff(args):
    ...
    if args.commit:
        # if a commit was provided, diff from it
        tree_from = base.get_tree(oid and base.get_commit(oid).tree)

    if args.cached:
        tree_to = base.get_index_tree()
        if not args.commit:
            # if not commit was provided, diff from HEAD
            oid = base.get_oid(data.HEAD)
            tree_from = base.get_tree(oid and base.get_commit(oid).tree)
    else:
...
def k(args):
    ...
    dot += '}'
    temp = open(f'{data.GIT_DIR}/graph.jpg', 'wb')
    with subprocess.Popen(
        ['dot', '-Tpng', '-o', temp.name],
    ...
    temp.close()
    os.system(os.path.abspath(temp.name))


def status(args):
    HEAD = base.get_oid(data.HEAD)
    branch = base.get_branch_name()
    if branch:
        print(f'On branch {branch}')
    else:
        print(f'{data.HEAD} detached at {HEAD[:10]}')
...
def push(args):
    remote.push(args.remote, f'{data.REFS}/{data.HEADS}/{args.branch}')
